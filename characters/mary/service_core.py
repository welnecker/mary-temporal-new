...
from .modules import memory_injection as mi
...
def _inject_intro_as_context_once(
    ...
    intro_text = mi._choose_intro_text(
                    usuario_key,
                    timeline,
                    get_fact_fn=get_fact,
                    sync_intro_fact_fn=_sync_intro_fact,
                )
    if intro_text:
...
def _normalize_reasoning_output(
    ...
    *,
    ...
    - interlocutor inválido (ex: 'Se', 'Muda', 'Minhas')
    ...
    if user_explicit_scene_change:
        ...
        r["proximo_passo_plausivel"] = (
            "reagir ao novo local e à mudança de cena sem recontar a cena anterior"
        )

    # ------------------------------------------------------
    # 5) continuidade imediata: encurta e impede conflito com facts
    # ------------------------------------------------------
...
class MaryService(BaseCharacter):
    ...
    def _resolve_turn_policy(
        ...
        *,
        ...
        # ==========================================================
        # Toggle de terceiros + profile
        # ==========================================================
        allow_third_party_seduction_final = third_party_enabled(
            usuario_key,
            third_party_override=allow_third_party_seduction,
            timeline=timeline_final,
        )

        allow_third_party_seduction_final = bool(allow_third_party_seduction_final and nsfw_on)
        diag.third_party_on = bool(allow_third_party_seduction_final)
        ...
        if nsfw_on and allow_third_party_seduction_final:
        ...
        # Fase íntima
        # ==========================================================
        intimacy_phase = _get_intimacy_phase(facts or {}, timeline_final)
        diag.phase_before = int(intimacy_phase)
        ...
        try:
            emotion_now = str(
                facts.get(f"emotion::{timeline_final}")
                or facts.get("emotion")
                or "neutro"
            ).strip() or "neutro"
        except Exception:
            emotion_now = "neutro"

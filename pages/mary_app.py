...
def main() -> None:
    ...
    # ✅ chave atual (use em tudo dentro da sidebar)
    usuario_key = _usuario_key_atual()
    ...
    st.caption(f"🔑 usuario_key atual: {usuario_key}")
    ...
    # ✅ Só reseta facts quando houver transição ON -> OFF (evita escrita a cada rerun)
    if nsfw_before and (not nsfw_after):
        st.session_state["mary_allow_third_party_seduction"] = False
        try:
    ...
    # ✅ Persistência do toggle NSFW (quando muda)
    ...
    if nsfw_after:
        ...
        # garante OFF no estado quando NSFW OFF
        st.session_state["mary_allow_third_party_seduction"] = False

    # ======================================================
    # Ciúme / Flerte / Segredo (somente NSFW)
    # ======================================================
    if nsfw_after:
        ...
        try:
            facts_rel = get_facts(usuario_key) or {}
        ...
        try:
            set_fact(usuario_key, "rel.ciume_flerte_segredo", (seed_input or "").strip(), {"fonte": "sidebar"})
            set_fact(usuario_key, "rel.ciume_cooldown_turns", int(cooldown_input), {"fonte": "sidebar"})
        except Exception:
            pass
